...
class ConfluenceResolver {
  constructor({ confluence, confluenceSpaces }) {
    ...
    const contentPattern = confluence.escapedBaseUrl +
      'spaces/(' + confluenceSpaces.spaceKeys.join('|') + ')' +
      '/pages/([0-9]+)(?=$|[^0-9]|\\s)'
    this.contentRegex = confluenceSpaces.spaceKeys.length == 0 ?  null : new RegExp(contentPattern, 'g')
  ...
  async resolve({ text, slackDomain, slackChannel }) {
    if (!this.contentRegex) return []

    const contentMatches = execall(this.contentRegex, text)
