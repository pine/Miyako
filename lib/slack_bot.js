...
class SlackBot {
  constructor({
    ...
    this.bitbucketResolver = bitbucketResolver
  ...
  async onMessage(message) {
    try {
      ...
      await Promise.all([
        this.processBitbucket({ message, channel }),
  ...
  async processBitbucket({ message, channel }) {
    const bitbucketContents = await this.bitbucketResolver.resolve({
      text: message.text,
      team: { domain: this.domain, channel },
    })
  }

  async processConfluence({ message, channel }) {
    const confluenceContents = await this.confluenceResolver.resolve({
      text: message.text,
    ...
    await this.confluencePostman.post(message.channel, message.thread_ts, postMessages.join('\n'))
  }
  ...
  async processJira({ message, channel }) {
    ...
    if (postMessages.length === 0) return
    await this.jiraPostman.post(message.channel, message.thread_ts, postMessages.join('\n'))
  }
}
